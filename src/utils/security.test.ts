 ...
 */
import { validators, rateLimit, SECURITY_CONSTANTS } from '@/utils/security';
...
describe('rateLimit', () => {
  ...
  it('blocks the request that exceeds the limit', () => {
    ...
    const maxRequests = 3;
    for (let i = 0; i < maxRequests; i++) {
      rateLimit(makeRequest(ip), maxRequests, SECURITY_CONSTANTS.RATE_LIMIT_WINDOW_MS, prefix);
    }
    const result = rateLimit(
      makeRequest(ip),
      maxRequests,
  ...
  it('resets after the window expires', () => {
    ...
    // Exhaust the limit
    rateLimit(makeRequest(ip), maxRequests, windowMs, prefix);
    rateLimit(makeRequest(ip), maxRequests, windowMs, prefix);
    const blocked = rateLimit(makeRequest(ip), maxRequests, windowMs, prefix);
    expect(blocked.allowed).toBe(false);

    // Advance past the window
    jest.advanceTimersByTime(windowMs + 1);

    const reset = rateLimit(makeRequest(ip), maxRequests, windowMs, prefix);
